...
fn compile(
    ...
    config
        ...
        .protoc_arg("grpc_api_configuration=proto/api_config_http.yaml,output_format=yaml,allow_merge=true,merge_file_name=contracts_info")
        .bytes(["."])
        .btree_map(["."])
        .type_attribute(".", "#[actix_prost_macros::serde]")
        .field_attribute(
            ".blockscout.contractsInfo.v1.HealthCheckRequest.service",
            "#[serde(default)]"
        );
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile(
        &["proto/contracts_info.proto", "proto/health.proto"],
        &["proto"],
        gens,
    )?;
    Ok(())
