...
pub async fn init_db(test_suite_name: &str, test_name: &str) -> TestDbGuard {
    ...
    // let db_url = Some("postgres://postgres:admin@localhost:10432/".into());
    let db_name_hash = // We use a hash, as the name itself may be quite long and be trimmed.
        Bytes::from(md5::compute(format!("{DB_PREFIX}_{test_suite_name}_{test_name}")).0);
    let db_name = format!("_{db_name_hash}");
