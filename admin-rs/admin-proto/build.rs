...
fn compile(
    ...
    config
        ...
        .protoc_arg("grpc_api_configuration=proto/api_config_http.yaml,output_format=yaml,allow_merge=true,merge_file_name=admin")
        .bytes(["."])
        .btree_map(["."])
        .type_attribute(".", "#[actix_prost_macros::serde]")
        .field_attribute(
            ".blockscout.admin.v1.ListTokenInfoSubmissionsRequest.status", 
            "#[serde(default)]"
        )
        .field_attribute(
            ".blockscout.admin.v1.TokenInfoSubmission.id", 
            "#[serde(default)]"
        )
        .field_attribute(
            ".blockscout.admin.v1.TokenInfoSubmission.status", 
            "#[serde(default)]"
        )
        .field_attribute(
            ".blockscout.admin.v1.TokenInfoSubmission.updated_at",
            "#[serde(default)]"
        )
        .field_attribute(
            ".blockscout.admin.v1.HealthCheckRequest.service",
            "#[serde(default)]"
        );
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    compile(
        &["proto/admin.proto", "proto/health.proto"],
        &["proto"],
        gens,
    )?;
    Ok(())
